...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if (array_key_exists('Authorization', $headers) && preg_match('/Bearer\s(\S+)/', $headers['Authorization'], $matches)){
		...
		if(verifyToken($matches[1])){
			...
			$class = mysqli_query($conn, "SELECT ClassRoomID,classrooms.Year,ClassRoomName,TeacherID,TeacherName FROM `classrooms` LEFT JOIN `teachers` ON `classrooms`.`ClassTeacher` = `teachers`.`TeacherID` WHERE classrooms.SchoolID = '$sid' AND classrooms.ClassTeacher IS NULL OR classrooms.SchoolID = '$sid' AND teachers.TeacherID IS NOT NULL");
